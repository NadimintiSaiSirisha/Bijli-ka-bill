...
$sql="update
  `$naam` t1
  join `master` t2 on t2.iotdevice = t1.appliance
set
  t1.12hr = t2.power
where
...
$sql="UPDATE `$naam` SET 10hr=$ans WHERE appliance='totalhr'";
mysqli_query($conn,$sql);
$sql = "SELECT SUM(11hr) as soma FROM `$naam` WHERE appliance!='totalhr'";
$result=mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
$row=$result->fetch_assoc();
$ans=$row["soma"];
